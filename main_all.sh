...
NVIDIA_SMI_PID=$!
python main.py -v --batch_size=$BATCH_SIZE --dataset='datasets/dataset_val/val' --network="$NETWORK" --less --weights weights/yolov8n-cls.pt
kill $NVIDIA_SMI_PID
...
NVIDIA_SMI_PID=$!
python main.py -v --batch_size=$BATCH_SIZE --dataset='datasets/dataset_val/val' --network="$NETWORK" -trt --engine='weights/best_fp32.engine' --less --non_verbose
kill $NVIDIA_SMI_PID
...
NVIDIA_SMI_PID=$!
python main.py -v --batch_size=$BATCH_SIZE --dataset='datasets/dataset_val/val' --network="$NETWORK" -trt --engine='weights/best_fp16.engine' --less --non_verbose
kill $NVIDIA_SMI_PID
