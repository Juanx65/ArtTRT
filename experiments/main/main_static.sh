...
if [ "$PROFILE" = "tegrastats" ]; then
    execute "$VANILLA" "jetson" "outputs/tegrastats_log/vanilla_${NETWORK}_bs_${BATCH_SIZE}_${POWER_MODE}_STATIC.txt"
    execute "$FP32" "jetson" "outputs/tegrastats_log/fp32_${NETWORK}_bs_${BATCH_SIZE}_${POWER_MODE}_STATIC.txt"
    execute "$FP16" "jetson" "outputs/tegrastats_log/fp16_${NETWORK}_bs_${BATCH_SIZE}_${POWER_MODE}_STATIC.txt"
    execute "$INT8" "jetson" "outputs/tegrastats_log/int8_${NETWORK}_bs_${BATCH_SIZE}_${POWER_MODE}_STATIC.txt"
else
    execute "$VANILLA" 
    execute "$FP32"
    execute "$FP16"
